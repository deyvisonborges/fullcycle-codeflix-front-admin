...
export function LisCastMembersPage() {
  ...
  const [castMembersData, setCastMembersData] = useState<CastMemberAPIModel[]>(
    []
  )
  ...
  useEffect(() => {
    if (data) setCastMembersData(data.data.flat())
  }, [data])
  ...
  return (
    <div>
      <table border={1}>
        ...
        <tbody>
          {castMembersData.map((castMember) => (
            <tr key={castMember.id}>
              <td>{castMember.id}</td>
              <td>{castMember.name}</td>
              <td>{castMember.type}</td>
              <td>
                <HiPencil
                  onClick={() =>
                ...
                &nbsp;
                <HiTrash
                  onClick={() => deleteCastMember({ id: castMember.id })}
                />
              </td>
            </tr>
          ))}
...
//                 />
//                 &nbsp;
//                 <HiTrash onClick={() => deleteCastMember({ id: category.id })} />
//               </td>
//             </tr>
//           ))}
//         </tbody>
